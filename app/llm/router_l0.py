...
_URGENT_PATTERNS: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    # «сч[её]ты» — вход бывает и без ё (golden set: «свести счеты с жизнью»).
    (_p(r"\bсвести\s+сч[её]ты\s+с\s+жизнью\b|\bпрощат?ься\s+с\s+жизнью\b"), "suicidal_intent"),
    ...
    (
        _p(
            r"\b((пере)?поре(жу|зать)|(пере)?режу|(пере)?резать|вскр(ою|ыть))\s+(себе\s+)?(вены|руки)\b"
            r"|\bпорежусь\b|\b(пойду|иду|буду)\s+резаться\b|\bрезаться\b"
            r"|\bсебя\s+поре(зать|жу)\b"
        ),
        "self_harm",
    ),
    ...
    (
        _p(
            r"\bповеш(усь|аюсь)\b|\bповесит?ься\b|\bпойду\s+вешаться\b"
            r"|\bгото(влю|ва|в)\s+вер[её]вк|\bвер[её]вк\w*\s+(и\s+)?(петл|сдела|повеш)"
            r"|\b(сдела(ю|л|ть)|завяжу|завязать)\s+петл[юя]\b"
        ),
        "suicidal_method",
    ),
    # Прыжок: и «собираюсь прыгнуть», и «буду прыгать с моста», и «пойду спрыгну».
    (
        _p(
            r"\b(собираюсь|пойду|буду)\s+(вы|с)?прыг\w+"
            r"|\b(вы|с)?прыгн\w*\s+(из\s+окна|с\s+балкона|с\s+моста|с\s+крыши)\b"
            r"|\bпрыга(ть|ю)\s+с\s+(моста|крыши|балкона)\b"
        ),
        "suicidal_method",
    ),
    (_p(r"\bнож\w*\s+(уже\s+)?(взял|в\s+рук|остр\w*\s+лежит|лежит\s+перед|приготовлен|наготове)"), "suicidal_method"),
    # Раздача вещей / оставленная записка — приготовления (совпадает с
    # safety-bench config/safety_lexicon.yml, tag prep_note).
    (
        _p(
            r"\bразда(ю|л|ла)\s+(свои\s+)?вещи\b|\bвещи\s+(свои\s+)?разда(ю|л|ла)\b"
            r"|\b(написал|оставил|пишу)\s+предсмертн\w+\s+(записк|письм)"
            r"|\b(записк\w*|письм\w*)\s+дет\w+\b.{0,45}(на\s+всякий\s+случай|вдруг\s+не\s+успе|на\s+стол|прощ)"
        ),
        "suicidal_method",
    ),
    # Самоотравление: «отравлюсь таблетками», «наглотаюсь таблеток», «приму
    # большую дозу», «выпью таблеток и усну навсегда» (golden set).
    (
        _p(
            r"\bотрав(люсь|иться|ить\s+себя)\b"
            r"|\bнаглота(юсь|ться)\s+(таблет|снотворн|лекарств)"
            r"|\bпри(му|нять)\s+(большую\s+дозу|смертельную\s+дозу)"
            r"|\bпри(му|нять)\s+(все|всю\s+пачку|горсть)\s+(таблет|лекарств)\w*"
            r"\s+(сразу|разом|за\s+раз|что\s+(есть|накопил)|которые\s+(есть|накопил))"
            r"|\bвыпь?ю\s+(все\s+|всю\s+пачку\s+|горсть\s+)?таблет\w*\s+и\s+(усн|засн|не\s+просн)"
        ),
        "suicidal_method",
    ),
    # «усну/засну навсегда», «уснуть и не просыпаться» — эвфемизм необратимого.
    (_p(r"\b(усн(у|уть)|засн(у|уть))\s+навсегда\b"), "suicidal_intent"),
    # Прощальное сообщение/письмо/записка — приготовление.
    (_p(r"\bпрощальн\w*\s+(сообщени|письм|записк|смс)"), "suicidal_method"),
    (_p(r"\bпопрощат?ься\s+со\s+всеми\b"), "suicidal_intent"),
    ...
    # «раз и навсегда» рядом с «поставить точку» / «покончить с этим» — тот же
    # эвфемизм необратимого завершения (golden set).
    (_p(r"\b(поставить\s+точк\w*|поконч(ить|у))\b.{0,25}\bраз\s+и\s+навсегда\b"), "suicidal_intent"),
    (_p(r"\bпора\s+кончать\s+с\s+(этим|жизнью)"), "suicidal_intent"),
    (_p(r"\bпрощаюсь\s+со\s+всеми\b"), "suicidal_intent"),
    # «что-то с собой сделать» — эвфемизм суицидальных мыслей. Требуем рядом
    # маркер намерения/мысли: без него «надо что-то с собой сделать» — про форму/вес.
    (
        _p(
            r"\b(мысл\w+|думаю|тянет|подмывает|хочется|ловлю\s+себя)\b[^?!]{0,45}"
            r"\b(что-?то\s+(с\s+)?собой|(с\s+)?собой\s+что-?то)\s+(сдела|поде)\w*"
            r"|\b(что-?то\s+(с\s+)?собой|(с\s+)?собой\s+что-?то)\s+(сдела|поде)\w*[^?!]{0,45}"
            r"\b(мысл\w+|думаю|тянет|подмывает)\b"
        ),
        "suicidal_intent",
    ),
)
...
# Последняя ветка — голое "если <орган/показатель> рухнет/встанет/забьётся":
# без "а"/"вдруг" ("если давление рухнет прямо на диализе, кто подбежит?" —
# patient-sim s05 ход 3). Глагол-катастрофа рядом убирает двусмысленность с
# бытовым "если что — звоните".
_HYPOTHETICAL_MARKER_RE = _p(
    r"\bа\s+вдруг\b|\bчто\s+если\b|\bчто\s+будет,?\s+если\b|\bа\s+если\b|\bесли\s+вдруг\b"
    r"|\bесли\s+(?:\w+\s+){0,3}(?:рухн\w*|встан\w*|останов\w*|забь[её]тся|"
    r"не\s+выдерж\w*|откаж\w*|порв[её]тся|лопн\w*|прихватит)"
)
...
# Катастрофизация здоровья: страх УМЕРЕТЬ ОТ болезни/осложнения, поданный
# гипотетически («а вдруг», «если не успеют»). Это тревога, не суицид — v2-рубрика
# safety относит такое к none. Ловится ТРИПЛЕТОМ (смерть-слово + гипотетическая
# рамка + медицинский контекст), чтобы не задеть ни реальный суицидальный сигнал
# («зачем всё это тянуть» — нет смерть-слова и мед-контекста), ни реальную
# мед-неотложку («давление рухнуло, кажется умру» — нет гипотетической рамки).
# Единственный потребитель — `_apply_agent_safety_net` (supervisor.py): понижает
# ЕДИНОЛИЧНЫЙ urgent агента до concern, когда ни L0, ни классификатор его не
# подтвердили. Найдено на patient-sim s05_anxious (2026-09-03).
_HEALTH_CATASTROPHE_DEATH_RE = _p(
    r"\bумр\w+|\bсмерт\w+|\bпомру\b|\bне\s+спас\w+|\bне\s+успе\w+|\bне\s+пережив\w+"
)
_HEALTH_CATASTROPHE_MED_RE = _p(
    r"\bшунт\w*|\bфистул\w*|\bкатетер\w*|\bосложнени\w+|\bкома\b|\bинсульт\w*"
    r"|\bинфаркт\w*|\bприступ\w*|\bдавлени\w+.{0,14}(?:рухн|упад|подск|скач|подним)"
    r"|\bпочк\w+.{0,14}отка\w+|\bскор[ао]\w*.{0,14}(?:помощ|не\s+успе)"
    r"|\bврач\w+.{0,18}(?:успе|не\s+успе)|\bночью\b|\bсознани\w+"
)


def looks_like_health_catastrophizing(text: str) -> bool:
    """Смерть-слово + гипотетическая рамка + мед-контекст в одном сообщении."""
    t = str(text or "")
    return bool(
        _HEALTH_CATASTROPHE_DEATH_RE.search(t)
        and _HYPOTHETICAL_MARKER_RE.search(t)
        and _HEALTH_CATASTROPHE_MED_RE.search(t)
    )
...
# Сон в parse_vitals НЕ попадает: sleep_tracker требует время отхода ко сну и
# подъёма, из «спал 3 часа» запись не собрать. Но отчёт о длительности сна —
# явная попытка внести данные: L0 распознаёт её отдельным интентом
# (``sleep_entry``), а pipeline отвечает кнопкой в трекер сна, без ложного
# «Записал» и без вызова модели.
_SLEEP_ENTRY_RE = _p(
    r"\b(?:спал\w*|поспал\w*|сон)\b[^?]{0,25}?"
    r"(?:\d{1,2}(?:[.,]\d)?\s*(?:час\w*|ч\b)|час\w*\s*\d{1,2})"
)

# Если рядом с бытовым отчётом (сон, распорядок) есть жалоба на состояние —
# это не «внеси в трекер», а повод для живого ответа. Детектор эмоций
# (_carries_emotion) такие короткие формулировки не всегда ловит.
_DISTRESS_RE = _p(
    r"\bразбит\w*|\bсил\w*\s+(совсем\s+)?нет\b|\bне\s+выспал\w*|\bвымот\w*"
    r"|\bужасно\b|\bкошмар\w*|\bтяжело\b|\bне\s+могу\b|\bплохо\s+себя\b"
)

# Отчёт о выполнении распорядка дня (МКФ d230): «соблюдал распорядок»,
# «справился с делами на день», «отметить день». Полную верификацию из чата
# не собрать (нужны активности из плана) — pipeline отвечает кнопкой в трекер
# распорядка, без вызова модели. Узко: только про сам распорядок/план, не про
# «рассказываю, как прошёл день».
_ROUTINE_VERB = r"(?:выполн\w+|соблюд\w+|прош[её]л\w*|справ\w+|сдела\w+|заверш\w+)"
_ROUTINE_ENTRY_RE = _p(
    # распорядок/рутина рядом с глаголом выполнения — в любом порядке
    rf"\b(?:распорядок|распорядк\w+|рутин\w+)\b[^?]{{0,30}}?\b{_ROUTINE_VERB}\b"
    rf"|\b{_ROUTINE_VERB}\b[^?]{{0,30}}?\b(?:распорядок|распорядк\w+|рутин\w+)\b"
    # «справился с делами на день», «сделал всё что планировал на сегодня»
    rf"|\b{_ROUTINE_VERB}\b[^?]{{0,25}}?\b(?:план\w*|дела\w*|задач\w+)\b[^?]{{0,15}}?"
    r"\bна\s+(?:сегодня|день)\b"
    # «отметить день / распорядок»
    r"|\b(?:отмет\w+|внест\w+|записа\w+)\b[^?]{0,20}?\b(?:день|распорядок|рутин\w+)\b"
)
_ROUTINE_NEGATION_RE = _p(r"\bне\s+(?:смог\w*|выполн\w+|получ\w+|справ\w+|уда\w+)")
...
def classify(
    ...
    *,
    ...
    if (
        _SLEEP_ENTRY_RE.search(message)
        and "?" not in message
        and not concern
        and not _DISTRESS_RE.search(message)
        and not _carries_emotion(message)
    ):
        # «спал 4 часа», «поспал часов 5» — отчёт о длительности сна.
        # Записать это как есть нельзя (нужно время отхода/подъёма), но и
        # отмалчиваться странно: pipeline ответит короткой репликой + кнопкой
        # в трекер сна. Эмоция/тревога рядом («спал 3 часа, всё валится») —
        # мимо: там нужен живой ответ модели, а не кнопка.
        return L0Decision(intent="sleep_entry", rule="sleep_duration_reported")

    if (
        _ROUTINE_ENTRY_RE.search(message)
        and "?" not in message
        and not concern
        and not _ROUTINE_NEGATION_RE.search(message)
        and not _DISTRESS_RE.search(message)
        and not _carries_emotion(message)
    ):
        # «соблюдал распорядок», «справился с делами на день» — отчёт о
        # выполнении распорядка. Полную верификацию из чата не собрать (нужны
        # активности из плана дня), поэтому — кнопка в трекер распорядка.
        return L0Decision(intent="routine_entry", rule="routine_completion_reported")
