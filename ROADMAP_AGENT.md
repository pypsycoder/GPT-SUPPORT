...
- [~] **Полный patient-sim на Cloud.ru** *(2026-09-03, 29 перефразировок)*:
  **24 PASS / 1 WARN / 4 FAIL**. Recall цел — s01 7/7, s02 6/6. Здоровье-тревога
  ...
  - `s05` ход 3 — (a) агент эскалирует на фрустрации без кризиса («дай конкретику
    ...
    `_is_hypothetical_question` L0 (гард ждёт «а если» / «если вдруг»);
...
допилить L0-гард на голое «если …рухнет/забьётся» + разнести кризис-шаблоны
psych/medical на ходе «пока скорая едет». **Прод переключается флагом/кнопкой —
решение за исследователем.**
