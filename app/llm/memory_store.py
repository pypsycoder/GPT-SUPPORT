"""
Персистентная память (шаг 5, new_agent_comcept/CLAUDE_CODE_BRIEF.md).
...
закрываются два оставшихся слоя памяти из ``00_MANUAL.md`` (часть 5.1):
...
# промпт бесконечно (00_MANUAL.md, часть 5.3).
FACT_TTL_DAYS = 45

# Потолок активных фактов на пациента: без него профиль пациента через
# месяцы бесконтрольной записи превращается в шум (00_MANUAL.md, часть 5.3).
MAX_ACTIVE_FACTS = 20
