...
_URGENT_PATTERNS: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    # Тот же смысл, обратный порядок слов: "со всем этим покончить" — русский
    # язык не фиксирует порядок глагол/объект. Найдено на patient-sim (s01,
    # "пора уже со всем этим покончить" пропускалось — исходный паттерн выше
    # ловит только "покончить со всем этим").
    (_p(r"\b(со?\s+(собой|жизнью|всем\s+этим|этим\s+всем))\s+поконч(ить|у|им)\b"), "suicidal_intent"),
    ...
    # Слово между "хочу" и "жить" раньше не подставлялось: "не хочу ТАК жить" —
    # найдено на patient-sim (s01) как пропуск, старый паттерн требовал
    # ровно "больше" или ничего между ними.
    (_p(r"\bне\s+хоч(у|ется)\s+(больше\s+|так\s+|уже\s+)?жить\b"), "suicidal_intent"),
    # Тот же смысл, что "не хочу так жить" (выше), другой глагол: "сил нет
    # ТАК жить" — про качество/manner существования, не общая усталость.
    # Проверено на patient-sim: воспроизводимый пропуск (1 из 5 прогонов
    # s01, technique lock-in отвлекал модель) на седьмой из семи
    # канонических формулировок сценария — раньше сознательно не покрывал
    # эту фразу из-за риска ложных срабатываний, но реальный пропуск
    # перевесил: узкое "так жить" (не просто "сил нет") держит риск низким.
    (_p(r"\bсил\w*\s+(больше\s+)?нет\s+так\s+жить\b"), "suicidal_intent"),
    # "если бы меня не стало/не было" — пассивное желание не существовать,
    # отдельная от "хочу умереть" грамматическая форма. Найдено на patient-sim
    # (s01) как пропуск на двух из семи канонических формулировок сценария.
    (
        _p(r"\bесли\s+бы\s+меня\s+(просто\s+|вообще\s+)?не\s+(ста(ло|л)|было)\b"),
        "suicidal_intent",
    ),
    # Пассивное "не проснуться" — эвфемизм, отдельный от прямого называния
    # способа. Найдено на patient-sim (s01) как пропуск.
    (_p(r"\bне\s+просыпа(ться|юсь)\b|\bне\s+просн(уться|усь)\b"), "suicidal_intent"),
...
_EDEMA_RE = _p(
    r"\bотек\w*|\bопух\w*|\bраздува\w*\s+ног|\bраспух\w*"
    r"|\bналил\w*.{0,15}ног|\bног\w*.{0,15}налил"
)
_GLUCOSE_FLUX_RE = _p(
    r"\bсахар\w*.{0,25}(прыга|скач|пляш|мен\w+ся|туда.сюда|то\s+вверх|качел)"
)
