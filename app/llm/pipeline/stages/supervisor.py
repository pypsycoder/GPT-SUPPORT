...
def _l0_note(decision) -> str:
    ...
    if decision.safety_level == "concern":
        ...
        elif rule == "alcohol_mention":
            lines.append(
                "Пациент упомянул, что употреблял алкоголь. Это не тема общего ЗОЖ, а "
                "отдельный клинический риск на диализе: алкоголь усиливает перепады "
                "давления и нагрузку на сосудистый доступ, взаимодействует с "
                "гипотензивными и другими препаратами, нарушает водно-электролитный "
                "баланс и уровень калия, может смазать симптомы осложнений. Ответь по "
                "существу с этой конкретикой, а не общей фразой «алкоголь вреден», и "
                "мягко уточни дозу/частоту и самочувствие сейчас — без нотаций."
            )
        else:
            lines.append(
