...
_EDUCATION_NEUTRAL_ACK_EXACT = frozenset({
    ...
    "принял", "приняла",
    "усвоил", "усвоила",
    # Одобрительные реакции без запроса продолжения: пациент доволен ответом,
    # но новой темы не поднимает. Без них education-сессия оставалась открытой
    # и эксперт выдавал ещё одну лекцию по той же теме.
    "круто", "супер", "класс", "отлично", "здорово", "хорошо",
})
