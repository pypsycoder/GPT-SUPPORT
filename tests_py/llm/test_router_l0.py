...
# --------------------------------------------------------------------------- #
# Алкоголь на диализе — клинический риск, не тема ЗОЖ
# --------------------------------------------------------------------------- #
...
@pytest.mark.parametrize(
    ...
    [
        "вчера выпил водки",
        "напился пива с друзьями",
        "принял пару рюмок коньяка",
        "выпила бокал вина на дне рождения",
    ],
)
def test_alcohol_consumption_raises_concern_as_medical(text):
    decision = router_l0.classify(text)

    assert decision.rule == "alcohol_mention"
    assert decision.safety_level == "concern"
    assert decision.safety_kind == "medical"
    assert decision.intent is None  # модель решает, интент L0 не присваивает


@pytest.mark.parametrize(
    "text",
    [
        "можно ли вообще алкоголь при диализе",  # тема без факта употребления
        "друг выпил вчера",                       # глагол есть, вещество не названо
        "купил вино в подарок",                   # вещество есть, глагола нет
        "у нас был отличный пилот на этом рейсе",  # корень "пил" внутри другого слова
    ],
)
def test_alcohol_pattern_requires_both_substance_and_verb(text):
    """Оба слова нужны вместе — иначе ловим упоминание темы или слова с тем же
    корнем («пилот»). Известный неустранимый остаток: «вина» (guilt) в
    именительном падеже омонимична вину (родительный «вина»/«вино») — при
    встрече с глаголом употребления («это моя вина, что я напился») сработает
    как ложное срабатывание. Цена приемлема (мягкая подсказка, не протокол)."""
    decision = router_l0.classify(text)
