...
7. **Не плодить корневые `.md`** — см. раздел «Документация» ниже.

---

## Документация

- **`ROADMAP_AGENT.md` (корень) — основной документ** по агентскому / LLM-контуру:
  фазы, спринты, статус, связь с находками аудита. Прогресс по агенту фиксируется
  правкой секции в нём, **не новым файлом**.
- **Не создавать новые корневые `.md`.** Если развёрнутый отчёт реально нужен
  отдельным файлом (чтобы не раздувать roadmap) — класть в **`docs/agent/`**, а в
  `ROADMAP_AGENT.md` добавлять ссылку под заголовком «Связанные документы».
- Допустимые корневые `.md`: `Readme.md`, `CLAUDE.md`, `AGENTS.md`,
  `ROADMAP_AGENT.md`, `ALEMBIC_RUNBOOK.md`, `SPRINT1_INVESTIGATIONS.md`.
- Пакеты материалов для доменных экспертов (нефрологи/психиатр) — вне репозитория
  (`.gitignore`), не коммитить.
- `AGENTS.md` (корень) — краткая точка входа для не-Claude агентов (Codex/Cursor),
  ссылается сюда; правится вместе с этим разделом.
