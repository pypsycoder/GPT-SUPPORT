...
> **Статус на 2026-08-30** (коммит `4e56384`): Спринт 1 закрыт по коду,
> **Фаза 2 закрыта по коду** (единый координатор + cutover), Фаза 4 начата
> (`education_cta`, рейт-лимит, `proactive_anomaly.txt`, STRUCTURE.md).
> Открыто: **staging-smoke Фаз 1–2** → снести старые `deliver_*`; затем routine
> из чата, `crisis_semantic`, сон из чата «правильно», миграция `account_id`.
...
- **Триггера «при первом входе» не существует.** Заготовка
  ...
  не подключена.
- **Канала доставки проактива нет.** Сообщение падает в `llm.chat_messages` и
  всплывает только при следующем визите пациента на веб-страницу (сайдбар грузит
  бейджи один раз на `DOMContentLoaded`, чат — историю один раз при открытии).
  Ни push, ни Telegram-доставки.
...
| Канал напоминаний | **Telegram** — *подтвердить: кода бота в репозитории нет (`app/bots/` отсутствует); в `.env` есть `BOT_TOKEN`, в `scheduler.py` фильтр `telegram_id IS NOT NULL`* | Фаза 3 = бридж `chat_messages → Telegram` (+ возможно сам бот-доставщик) |
...
потом включить проактив для сценария «вход пациента» (канал доставки не нужен —
сообщение ждёт в истории); потом сделать проактив связным; настоящий
Telegram-канал для напоминаний «не в момент входа» — отдельная крупная фаза.
...
### Фаза 3 — Telegram-канал доставки

**Для:** напоминаний не в момент входа (таблетки в 14:00, алерты по аномалиям).

- [ ] Проверить / создать бота-доставщика (`app/bots/` в репозитории отсутствует).
- [ ] Бридж: новое proactive-сообщение в `llm.chat_messages` → отправка в Telegram
  по `users.telegram_id`.
- [ ] Единый `is_read` для «прочитано в Telegram» и «прочитано в вебе».
- [ ] Тихие часы, антиспам (суммарный потолок N/день вместе с координатором Фазы 2).

**Оценка:** 2–3 спринта. Крупнейшая фаза, кандидат в отдельный эпик.

### Фаза 4 — Реактивные пробелы и гигиена

- [x] **Сон из чата — минимум (сделано в спринте 1, #6).** SLEEP убран из
  `router_l0.parse_vitals` + `data_entry` (`_LABELS`, `_render_value`). «Спал 3
  часа» больше не отвечает ложным «Записал: сон 3 ч» — уходит в общий пайплайн.
- [ ] **Сон из чата — правильно.** Лёгкий уточняющий диалог «во сколько лёг /
  встал» → запись в `sleep.sleep_records` через мост по образцу `vitals_writer`.
...
`get_daily_context_for_llm`, Telegram-канал, `education_cta`, routine из чата,
...
| ⬜ | Канала доставки проактива нет | 3 |
| 🟡 | Сон из чата: «сказал записал — не записал» | 1 (stretch) — ложный «Записал» убран ✅; уточняющий диалог + запись → 4 |
