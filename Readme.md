...
- `handlers/start.py` — обработчик `/start`: ищет или создаёт пользователя, показывает inline-меню и оставляет reply-клавиатуру быстрых действий после проверки согласия.
- `handlers/consent.py` — реакции на inline-кнопки согласия: обновляет флаг в базе и открывает inline-меню с быстрыми действиями.
...
- `keyboards/reply_start.py` — reply-клавиатура с быстрыми действиями (опрос, показатели).
