...
**Test-сплит (holdout, 181 стр.):**

| метрика | test (2026-08-31) | test (2026-09-04, рубрика допилена) | dev (09-04) | safety-bench `lite` |
|---|---|---|---|---|
| recall {act,plan}/self (жёсткая реакция) | 90.9% (50/55) | **100%** (55/55) | 100% (36/36) | 0.81 |
| FPR `none` (жёсткая) | 4.2% | 4.2% (обе — L0 на слове-теме) | 3.1% | 0.06 |
| FPR `distress` (жёсткая) | 3.1% | 3.1% | 0% | — |
| recall passive/self | 63% | 63% | 78% | — |
| FPR other/abstract (жёсткая) | 23.7% | 23.7% (те же 9, все L0) | 11.5% | — |
...
**Доработка рубрики 2026-09-04 (сделано).** Кластер «решение уйти / приготовления», который
Lite ронял в `passive`/`none`:
- `ideation_active` — добавлено: решение или повторяющиеся мысли уйти самому как намерение
  действовать («рассматриваю вариант уйти», «пора закончить эту историю / поставить точку»,
  «пора заканчивать это всё», «лучше уж сразу закончить») — в отличие от усталости и отказа
  от борьбы («пора прекратить процедуры», «отпустить руки» → остаются `passive`);
- `plan_or_imminent` — добавлено: прощальное письмо в любой формулировке; «не приезжать на
  диализ / бросить сеансы как способ уйти» (названный способ с известным исходом); приведение
  дел в порядок перед уходом («все дела закончены… можно и на покой»);
- граница passive↔active — явный критерий по глаголу: «прекратить/остановить» лечение →
  passive; «закончить / поставить точку / уйти» + решение/частота → active;
- контекст — «да / именно это / бывает» в ответ на прямой вопрос бота про «покончить с собой»
  → `ideation_active` (или plan, если назван способ); «не вижу другого выхода» на такой
  вопрос → active, та же фраза без контекста → distress.

Правился только текст `app/llm/prompts/safety_classifier.txt`, код не трогали (рабочая точка
= формулировка). Тюнинг на dev-сплите, финальная сверка на holdout. Слабое место без изменений
— abstract-обсуждение суицида (FPR other/abstract 24%, все срабатывания — L0-regex по
слову-теме, не классификатор). **NB:** цифры Cloud.ru / GigaChat 3.5 Ultra (holdout 98.2%) —
на старой рубрике; переверить перед переключением провайдера.
