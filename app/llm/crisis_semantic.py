...
# Откалибровано leave-one-out на mined+drafted корпусе (см.
# calibrate_crisis_semantic.py, отчёт в ночном журнале, 2026-08-27):
# margin=0.00 дал 100% recall / 0% ложных на leave-one-out, но это тест
# корпуса на самом себе — оптимистичная оценка. margin=0.01 — небольшой
# запас против голого тай-брейка, recall почти не теряет (резкое падение
# начинается только с margin=0.02, где recall падает до 82%). threshold
# в диапазоне 0.65-0.85 не менял результат вообще на этом корпусе — взят
# консервативно низкий край диапазона. НЕ проверено на трафике за
# пределами этого корпуса сверх того, что описано в ночном журнале —
# нужна повторная калибровка при появлении реальных данных.
DEFAULT_THRESHOLD = float(os.getenv("CRISIS_SEMANTIC_THRESHOLD", "0.70"))
DEFAULT_MARGIN = float(os.getenv("CRISIS_SEMANTIC_MARGIN", "0.01"))
