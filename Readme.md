...
Проект использует PostgreSQL и разделяет данные по схемам. Сейчас в рабочей базе (hemo_db) используются минимум три схемы:
...
Такое разделение сделано, чтобы потом можно было безболезненно добавлять новые домены (education, vitals, rehab) и давать к ним доступ разным сервисам.
...
- `main.py` — точка входа Telegram-бота: создаёт `Bot` и `Dispatcher`, регистрирует роутеры, при старте создаёт схемы `users`/`scales` и таблицы по общему `Base`, затем запускает polling.
...
- `handlers/start.py` — обработчик `/start`: ищет или создаёт пользователя, показывает клавиатуру согласия при отсутствии разрешений и приветствует при повторном визите.
- `handlers/consent.py` — реакции на inline-кнопки согласия: устанавливает флаг в базе и показывает стартовую клавиатуру при согласии, либо информирует об отказе.
- `handlers/questionnaire.py` — подключает FSM опроса из модуля шкал к маршрутам бота.
- `keyboards/start_keyboard.py` — reply-клавиатура с быстрыми действиями (опрос, показатели).
