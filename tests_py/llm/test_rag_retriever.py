...
async def test_retrieve_relevant_modules_with_meta_reports_python_backend(monkeypatch):
    ...
    assert "selection_reason" in result["modules"][0]
    assert "hybrid_score" in result["modules"][0]
    assert result["debug"]["selected_candidates"][0]["lesson_id"] == 10
...
async def test_retrieve_relevant_modules_with_meta_reports_pgvector_backend(monkeypatch):
    ...
    assert result["modules"][1]["lesson_id"] == 30
    assert result["debug"]["selected_candidates"][0]["lesson_id"] == 30
