(function () {
  ...
  function getRouterCard(diagnosticsJson) {
    var diagnostics = diagnosticsJson || {};
    return diagnostics.supervisor || null;
  }
  ...
  function renderRouterCard(turn) {
    ...
    var agentCard = supervisor.agent || null;
    var selectedAgents = supervisor.selected_agents || [];
    var summary = [];
    var details = [];

    if (!agentCard && !graphPath.length) {
      return '';
    }

    if (agentCard && agentCard.intent) summary.push('intent: ' + agentCard.intent);
    if (agentCard && agentCard.safety_level && agentCard.safety_level !== 'none') summary.push('safety: ' + agentCard.safety_level);
    ...
    if (agentCard) {
      if (agentCard.intent) details.push('<div><strong>Intent:</strong> ' + escHtml(agentCard.intent) + '</div>');
      if (agentCard.technique_id) details.push('<div><strong>Техника:</strong> ' + escHtml(agentCard.technique_id) + '</div>');
      if (agentCard.safety_level) details.push('<div><strong>Safety level:</strong> ' + escHtml(agentCard.safety_level) + '</div>');
      if (agentCard.safety_kind) details.push('<div><strong>Safety kind:</strong> ' + escHtml(agentCard.safety_kind) + '</div>');
      if (agentCard.safety_reason) details.push('<div><strong>Safety reason:</strong> ' + escHtml(agentCard.safety_reason) + '</div>');
      if (agentCard.next_action) details.push('<div><strong>Next action:</strong> ' + escHtml(agentCard.next_action) + '</div>');
      if (Array.isArray(agentCard.memory_candidates) && agentCard.memory_candidates.length) {
        details.push('<div><strong>Кандидаты в память:</strong> ' + escHtml(agentCard.memory_candidates.join(', ')) + '</div>');
      }
    }
